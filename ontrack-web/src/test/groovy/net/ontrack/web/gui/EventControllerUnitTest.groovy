...
class EventControllerUnitTest {
	...
	void createLinkHref_branch () {
		...
		assert """gui/project/PROJ6/branch/BRANCH1""" == href
	}
	...
	void createLinkHref_build () {
		...
		assert """gui/project/PROJ7/branch/BRANCH2/build/BUILD1""" == href
	}
	...
	void createLink_branch() {
		...
		assert """<a class="event-entity" href="gui/project/PROJ3/branch/1.x">1.x</a>""" == href
	}
	...
	void expandToken_entity_project () {
		EventController controller = dummy()
		def value = controller.expandToken('$PROJECT$', new ExpandedEvent(1, "Author", EventType.PROJECT_CREATED, new DateTime()).withEntity(Entity.PROJECT, new EntityStub(Entity.PROJECT, 1, "PROJ1")))
	...
	void expandToken_entity_project_group () {
		EventController controller = dummy()
		def value = controller.expandToken('$PROJECT_GROUP$', new ExpandedEvent(1, "Author", EventType.PROJECT_GROUP_CREATED, new DateTime()).withEntity(Entity.PROJECT_GROUP, new EntityStub(Entity.PROJECT_GROUP, 1, "GROUP1")))
	...
	void expandToken_entity_project_group_with_alternative () {
		EventController controller = dummy()
		def value = controller.expandToken(
                '$PROJECT_GROUP|this group$',
                new ExpandedEvent(
                        1,
                        "Author",
                        EventType.PROJECT_GROUP_CREATED,
                        new DateTime()
                ).withEntity(Entity.PROJECT_GROUP, new EntityStub(Entity.PROJECT_GROUP, 1, "GROUP2")))
	...
	void expandToken_entity_project_not_found () {
		EventController controller = dummy()
		controller.expandToken('$PROJECT$', new ExpandedEvent(1, "Author", EventType.PROJECT_CREATED, new DateTime()))
	}
	
	@Test
	void expandToken_value_only () {
		EventController controller = dummy()
		def value = controller.expandToken('$project$', new ExpandedEvent(1, "Author", EventType.PROJECT_DELETED, new DateTime()).withValue("project", "My > project"))
	...
	void toGUIEvent_one_entity () {
		...
		def event = controller.toGUIEvent(new ExpandedEvent(10, "Author", EventType.PROJECT_CREATED, new DateTime(2013,1,30,10,5,30)).withEntity(Entity.PROJECT, new EntityStub(Entity.PROJECT, 1001, "PROJ2")), Locale.ENGLISH, new DateTime(2013,1,30,11,10,45))
		assert event != null
		assert event.id == 10
		assert event.eventType == EventType.PROJECT_CREATED
		assert event.timestamp == "Jan 30, 2013 10:05:30 AM"
		assert event.elapsed == "1 hour ago by Author"
	...
	void toGUIEvent_two_entities () {
		...
		assert event.timestamp == "Jan 30, 2013 10:05:30 AM"
		assert event.elapsed == "1 hour ago by Author"
		assert 'Branch <a class="event-entity" href="gui/project/PROJ1/branch/1.x">1.x</a> has been created for the <a class="event-entity" href="gui/project/PROJ1">PROJ1</a> project.' == event.html
	}
