...
public interface SQL {
    ...
    String VALIDATION_RUN_HISTORY = "(SELECT VR.ID AS VRID, NULL AS STATUS, CONTENT, AUTHOR, AUTHOR_ID, COMMENT_TIMESTAMP AS EVENT_TIMESTAMP\n" +
            "FROM COMMENT C\n" +
            "INNER JOIN VALIDATION_RUN VR ON VR.ID = C.VALIDATION_RUN\n" +
            "INNER JOIN BUILD B ON B.ID = VR.BUILD\n" +
            "WHERE B.BRANCH = :branch AND VR.VALIDATION_STAMP = :validationStamp)\n" +
            "UNION\n" +
            "(SELECT VR.ID, VRS.STATUS, VRS.DESCRIPTION AS CONTENT, VRS.AUTHOR, VRS.AUTHOR_ID, VRS.STATUS_TIMESTAMP AS EVENT_TIMESTAMP\n" +
            "FROM VALIDATION_RUN_STATUS VRS\n" +
            "INNER JOIN VALIDATION_RUN VR ON VR.ID = VRS.VALIDATION_RUN\n" +
            "INNER JOIN BUILD B ON B.ID = VR.BUILD\n" +
            "WHERE B.BRANCH = :branch AND VR.VALIDATION_STAMP = :validationStamp)\n" +
            "ORDER BY VRID DESC, EVENT_TIMESTAMP DESC\n" +
